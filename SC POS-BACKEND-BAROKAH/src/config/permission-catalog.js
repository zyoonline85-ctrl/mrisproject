...
const permissionCatalog = [
  ...
  { key: "inventory.transfers", group: "inventory", label: "Transfer Stok", route: "/inventory/transfer-stok", actions: ["view", "create", "update", "approve", "reject"] },
  {
    ...
    actions: ["view", "create", "export", "approve", "reject"]
  },
  ...
  { key: "settings.profile", group: "settings", label: "Profil Akun", route: "/pengaturan/profil-akun", actions: ["view"] },
  {
    ...
    route: "apk://kasir",
    actions: ["view", "create", "update", "cancel", "print"]
  },
...
function createFullPermissionMap() {
...
function createApkFullPermissionMap() {
...
function hasPermissionGroupAccess(roleOrPermissions, predicate) {
  const permissions = roleOrPermissions?.permissions || roleOrPermissions || {};
  return Object.entries(permissions).some(
    ([key, actions]) => predicate(key) && Array.isArray(actions) && actions.includes("view")
  );
}

function hasApkAccess(roleOrPermissions) {
  return hasPermissionGroupAccess(roleOrPermissions, (key) => key.startsWith("apk."));
}

function hasAdminAccess(roleOrPermissions) {
  return hasPermissionGroupAccess(roleOrPermissions, (key) => !key.startsWith("apk."));
}
